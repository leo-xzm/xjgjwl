...
namespace POSApplication.UI
{
    public partial class frmRef : Form
    {
        ...
        public frmRef(Staff staff, frmMain main)
        {
            InitializeComponent();
            this.staff = staff;
            this.main = main;
        ...
        private void dgvGoods_Add(int number, decimal price)
        {
            ...
            if (code.Length == 13 && code.Substring(0, 2) == "24")
            {
                ...
                int checkCode = Convert.ToInt32(code.Substring(12, 1));
                //根据EAN-13规则计算内码
                int result = ((ean13[0] + ean13[2] + ean13[4] + ean13[6] + ean13[8] + ean13[10])
                    + (ean13[1] + ean13[3] + ean13[5] + ean13[7] + ean13[9] + ean13[11]) * 3) % 10;
                ...
                else
                {
                    ...
                    goods = new GoodsBLL().GetGoodsByCID(goodsCode);
                    //称重码自带价格
                    price = Convert.ToDecimal(code.Substring(7, 3)) + Convert.ToDecimal(code.Substring(10, 2)) / 100;
                }
            }
            else
            {
